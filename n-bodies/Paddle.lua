...
function Paddle:aiUpdate(dt, destination_y, paddle_speed)

    if self.dy < 0 then
        self.y = math.max(destination_y, math.max(0, self.y + self.dy * dt))
    else
        self.y = math.min(destination_y, math.min(VIRTUAL_HEIGHT - self.height, self.y + self.dy * dt))
    end
